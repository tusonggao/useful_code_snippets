...
password=bigdata123!@#
host=rm-wz9p44hb3k4n3xuj3to.mysql.rds.aliyuncs.com
	#地域分布表
	python /opt/datax/bin/datax.py -p"-Dusername=$username -Dpassword=$password -Dhost=$host  -DpreSql='truncate table area'"   area.json

	#地区分布表
	python /opt/datax/bin/datax.py -p"-Ddt=$yes_date -Dusername=$username -Dpassword=$password -Dhost=$host  -DpreSql='delete from product_user_distributed_daily where  create_time='\''$yes_date 00:00:00'\'' '"   	product_user_distributed_daily.json
	...
	#小时产品流量表
	python /opt/datax/bin/datax.py -p"-Ddt=$yes_date -Dusername=$username -Dpassword=$password -Dhost=$host  -DpreSql='delete from product_flow_hourly where  date(create_time)='\''$yes_date'\'' '" 				product_flow_hourly.json
	...
	#产品日退单表 （汇总）
	python /opt/datax/bin/datax.py -p"-Ddt=$yes_date -Dusername=$username -Dpassword=$password -Dhost=$host -DpreSql='delete from product_refund_daily where  create_time='\''$yes_date 00:00:00'\'' '" 				product_refund_daily.json
	...
	#产品关键词搜索转化表 
	python /opt/datax/bin/datax.py -p"-Ddt=$yes_date -Dusername=$username -Dpassword=$password -Dhost=$host -DpreSql='delete from product_search_word_daily where  create_time='\''$yes_date 00:00:00'\'' '" 			product_search_word_daily.json
	...
	#人群订单表
	python /opt/datax/bin/datax.py -p"-Ddt=$yes_date -Dusername=$username -Dpassword=$password -Dhost=$host -DpreSql='truncate table user_group'" 			user_group.json
