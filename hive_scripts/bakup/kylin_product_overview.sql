create view if not exists  views.product_overview
as 
select 
cast(nvl(t1.event_date,t7.order_date) as timestamp) as create_time,
nvl(t1.product_code,t7.product_code) as product_code,
nvl(p.product_name,t7.product_name) as product_name,
concat('http://image.jianke.com',nvl(pi.image_url,t7.image_url)) as product_img,
nvl(nvl(dpc.large_class,t7.large_class),'') as product_large_class,
nvl(nvl(dpc.small_class,t7.small_class),'') as product_small_class,
...
left join jkbd.dim_product_class dpc on t1.product_code = dpc.product_code
left join jkbd.dim_product p on t1.product_code = p.product_code
left join (select * from jkbd.ods_product_images_parquet where is_main=true) pi on t1.product_code = pi.product_code
left join (
...
(
    ...
    left join (select * from jkbd.ods_product_images_parquet where is_main=true) pi on t5.product_code=pi.product_code
    left join 
    (
    select 
            product_code,
    ...
    (
        ...
        where order_date >= '2019-01-01'
        	...
        	AND EL_RECORD_STATE = 2 
				...
				AND product_code!=857932
        group by order_date
    ) t6 on t1.event_date=t6.order_date
  left join   
    (
        select  
            order_date,
        ...
        where order_date >= '2019-01-01'
        	...
        	AND EL_RECORD_STATE = 2 
				...
				AND product_code!=857932
        group by order_date
    ) t61 on t7.order_date=t61.order_date
    ;
